...
if (nrow(rna_sub) == 0) {
  ...
  # adjust dot size and figure size
  p1 <- p +
    geom_hline(yintercept = 0, color = "darkgrey", linetype = 2) +
    geom_vline(xintercept = 0, color = "darkgrey", linetype = 2) +
    geom_point_rast(color = "#F15E75", size = 0.5, scale = 1, raster.dpi = 600) +
    geom_text_repel(
      aes(label = gene),
      data = res %>%
        filter(
          abs(model_log2FC) > 2.5 | abs(Log2FC) > 0.5,
          model_log2FC * Log2FC > 0
        ),
      size = 1
    ) +
  ...
  ggsave(paste0(
    "./plot/ALS_vs_Control_snRNA_expFC_vs_snATAC_genenScoreFC_in_", selected_cell_type, "_",
    selected_region, "_FC_", FC_threshold, "_FDR_", FDR_threshold, "_rasterized.pdf"
  ),
  plot = p1, device = cairo_pdf(), width = 2.5, height = 2.5, useDingbats = FALSE
  )
}
