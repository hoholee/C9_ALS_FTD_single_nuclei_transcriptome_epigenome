...
# split control subjects into two groups
control_subject_group_1 <- c("902", "904", "906")
control_subject_group_2 <- c("1069", "91", "945")
...
run_MAST <- function(selected_cluster, selected_region, selected_disease_1, selected_disease_2) {
  message(paste0(
    "Processing: ", selected_cluster, ", ", selected_region, ", ",
  ...
  if (selected_disease_1 == "ALS") {
    control_group_to_exclude <- control_subject_group_2
  } else if (selected_disease_1 == "FTD") {
    control_group_to_exclude <- control_subject_group_1
  }
  sub_idx <- which(
    meta_data$rna_anno_2ndRound_level_2 == selected_cluster &
      meta_data$region == selected_region &
      meta_data$disease != excluded_disease &
      !(meta_data$subject %in% control_group_to_exclude)
  )
  ...
  saveRDS(zlmCond, paste0(
    "./zlm_rds_splitControls/MAST_zlmFit_level2_", selected_cluster, "_", selected_region, "_",
    selected_diseases[1], "_vs_", selected_diseases[2], "_splitControls.rds"
  ))
  # save convergence
  ...
  saveRDS(summary_cond, paste0(
    "./zlm_rds_splitControls/MAST_zlmSummary_level_2_", selected_cluster, "_", selected_region, "_",
    selected_diseases[1], "_vs_", selected_diseases[2], "_splitControls.rds"
  ))
  ...
  write_tsv(final_res, paste0(
    "./results_splitControls/MAST_res_level2_", selected_cluster, "_", selected_region, "_",
    selected_diseases[1], "_vs_", selected_diseases[2], "_splitControls.txt"
  ))
