# run this in the `r_env_v4`conda env, with `MAST` v1.17.3 installed
library(tidyverse)
library(data.table)
library(NMF)
library(rsvd)
...
indi_FTD <- c("36", "908", "54", "55", "674", "61")
...
data_obj_sub <- readRDS("../snRNA_dataset_Seurat_object_rmDoublets_hardCutoff_scTransform_renormalized_PCA50_clustered_annotated.rds")

rna_anno <- read_tsv("annotation_all.txt")

meta_data <- data_obj_sub@meta.data %>%
  as_tibble() %>% 
  left_join(rna_anno) %>% 
  mutate(rna_anno_level_3 = if_else(is.na(rna_anno_level_3), "Glu_cerebellar_neu", rna_anno_level_3))

data_obj_sub@meta.data <- meta_data %>% as.data.frame()
rownames(data_obj_sub@meta.data) <- meta_data$barcode

# convert the Seurat object to the SummarizedExperiment class
# subset by cluster and by region
# using the DefaultAssay of the Seurat object ("SCT" in this case)
# and use the raw counts in the `counts` slot
selected_assay <- DefaultAssay(data_obj_sub)
...
run_MAST <- function(selected_cluster, selected_region, selected_disease_1, selected_disease_2){
  ...
  sub_idx <- which(meta_data$rna_anno_level_3 == selected_cluster &
  ...
  write_tsv(final_res, paste0("./results_shuffle/MAST_shuffle_res_", selected_cluster, "_", selected_region, "_",
                              selected_diseases[1], "_vs_", selected_diseases[2], ".txt"))
  
  toc()
  rm(sca, scaRaw, data_obj_used)
}

# test run
# run_MAST("Astro_PLCG1", "MCX", "ALS", "Control")

param <- expand_grid(
  selected_cluster = c("Astro_AQP1", "Astro_GRIA1", "Astro_PLCG1"),
