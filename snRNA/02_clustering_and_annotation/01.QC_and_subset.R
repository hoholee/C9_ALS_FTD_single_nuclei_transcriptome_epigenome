...
# VlnPlot(data_obj, features = c("nFeature_RNA", "nCount_RNA", "percent_mt"), ncol = 3)
## Ridge plot
ridge_nFeature <- RidgePlot(data_obj, "nFeature_RNA", log = TRUE)
ridge_nCount <- RidgePlot(data_obj, "nCount_RNA", log = TRUE)
ridge_percent_mt <- RidgePlot(data_obj, "percent_mt")
CombinePlots(plots = list(ridge_nFeature, ridge_nCount, ridge_percent_mt), legend = "none", ncol = 3)
ggsave("./plots/QC_all_samples_cellBender_corrected_nFeature_nCount_percentMT_ridge_plot.pdf",
  device = cairo_pdf(),
  width = 12, height = 8, useDingbats = F
)

## correlation plot
# nCount_vs_percentMt <- FeatureScatter(data_obj, feature1 = "nCount_RNA", feature2 = "percent_mt")
# nCount_vs_nFeature <- FeatureScatter(data_obj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
# CombinePlots(plots = list(nCount_vs_percentMt, nCount_vs_nFeature))

p <- meta_data %>% ggplot(aes(log10(nCount_RNA), log10(nFeature_RNA)))
p + geom_point(aes(color = percent_mt), size = 0.2) +
  facet_wrap(~orig.ident) +
  scale_color_viridis_c(limit = c(0, 5), oob = squish) +
  annotation_logticks() +
...
ggsave("./plots/QC_all_samples_cellBender_corrected_correlation_plot_nCount_vs_nFeature.pdf",
  device = cairo_pdf(),
  width = 12, height = 8, useDingbats = F
)
...
write_tsv(meta_data_add_doubletScores, "./meta_preQC_all_cells_post_cellBender.txt")
...
#                        subset = nFeature_RNA > nFeature_lower & nFeature_RNA < nFeature_upper & percent_mt < percent_mt_upper)
# data_obj_sub <- subset(data_obj,
#                        subset = nFeature_RNA > nFeature_lower & percent_mt < percent_mt_upper)
data_obj_sub <- subset(data_obj,
...
p <- meta_data_sub %>% ggplot(aes(log10(nCount_RNA), log10(nFeature_RNA)))
p + geom_point(aes(color = percent_mt), size = 0.2) +
  facet_wrap(~orig.ident) +
...
# ggsave(paste0("./plots/QC_all_samples_postFilter_nFeature_", nFeature_lower,
#               "_percentMT_", percent_mt_upper, "_correlation_plot_nCount_vs_nFeature.pdf"),
#               device = cairo_pdf(), width = 12, height = 8, useDingbats = F)
ggsave(
  paste0(
    "./plots/QC_all_samples_cellBender_corrected_postFilter_nFeature_", nFeature_lower,
    "_percentMT_", percent_mt_upper, "_doubletScore_", doublet_scores_upper, "_correlation_plot_nCount_vs_nFeature.pdf"
  ),
  device = cairo_pdf(), width = 12, height = 8, useDingbats = F
)
...
write_tsv(cell_count_merge, "num_cells_pass_QC_cellBender_corrected.txt")
...
ggsave(
  "./plots/preQC_cumulative_distribution_percent_mt_by_individual.pdf",
  device = cairo_pdf(),
  width = 3, height = 3, useDingbats = FALSE
)
...
# save Seurat object
saveRDS(data_obj_sub, file = "./seurat_objects/snRNA_cellBender_corrected_postQC_SeuratV4_object.rds")
